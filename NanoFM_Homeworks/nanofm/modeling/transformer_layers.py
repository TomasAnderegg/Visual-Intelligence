...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask : Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        q, v, k = [rearrange(W_i(x), 'b s (h d) -> b h s d', h = self.num_head) for W_i in (self.w_q, self.w_v, self.w_k)]
...
class Block(nn.Module):
    ...
    def __init__(self, dim: int, head_dim: int = 64, mlp_ratio: float = 4., use_bias: bool = False):
        super().__init__()
        self.norm1 = nn.LayerNorm(dim) # TODO (use the LayerNorm defined above)
        self.attn = Attention(dim, head_dim, qkv_bias=use_bias, proj_bias=use_bias) # TODO
        self.norm2 = nn.LayerNorm(dim) # TODO (use the LayerNorm defined above)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp(dim, dim, mlp_hidden_dim, bias=True) # TODO
