...
class VisionLanguageModel(nn.Module):
    ...
    def generate(self, input_ids, image, attention_mask=None, max_new_tokens=5):
        ...
        if attention_mask is not None:
            image_attention_mask = torch.ones(batch_size, img_seq_len, device=input_ids.device) # hint: we want all image tokens to be attended
    ...
    def generate_with_kv_cache(self, input_ids, image, attention_mask=None, max_new_tokens=5):
        ...
        for i in range(1, max_new_tokens):
            ...
            if (next_token.squeeze(-1) == 2).all(): # stop the generation loop if the generated token == 2 (token id 2 is the EOS token we used during training)
                break
