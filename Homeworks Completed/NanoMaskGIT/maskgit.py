...
class MaskGIT(nn.Module):
    ...
    def __init__(
        ...
        self.positional_embedding = nn.Parameter(torch.randn(seq_len, dim) * self.init_std) # TODO: Define the learnable positional embedding
        self.mask_token = nn.Parameter(torch.randn(dim) * self.init_std) # TODO: Define the learnable mask token
    ...
    def forward_model(self, x: torch.LongTensor, mask: torch.BoolTensor) -> torch.Tensor:
        ...
        x[mask] = self.mask_token #comme x [B, L, D] et mask [B, L]
    ...
    def generate_random_mask(self, seq: torch.Tensor) -> torch.BoolTensor:
        ...
        rand_perm = torch.rand(B, L, device=seq.device).argsort(dim=1) #on genere un tenseur de taille [B,L] de nombre aleatoire entr 0 et 1. puis argsortt retounr les indice qui trierait les valeurs # (B, L)
        num_masked = torch.randint(1, L + 1, (B,), device=seq.device)  # (B,) #selectionne combien de tokens masquer par sequence.

        mask = rand_perm < num_masked.unsqueeze(1)  # (B, L)
    ...
    def compute_ce_loss(self, logits: torch.Tensor, target_seq: torch.LongTensor, ignore_index: int = -100) -> torch.Tensor:
        ...
        loss = F.cross_entropy(logits, target_seq, ignore_index=ignore_index)
        return loss
    ...
    def get_maskgit_schedule(self, mask: torch.BoolTensor, num_steps: int = 8) -> List[int]:
        ...
        for i in range(num_steps):
            schedule.append(total_tokens // num_steps)
    ...
    def generate(
        ...
        for step, k in enumerate(schedule):
            ...
            masked_indices = mask[0].nonzero().squeeze(1)  #(M,) 

            # TODO: Get the logits for the `masked_indices` positions. Shape: [M, vocab_size]
            masked_logits = logits[0, masked_indices] # logits[1, L, vocab_size]
