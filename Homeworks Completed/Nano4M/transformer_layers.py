...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask : Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        q, v, k = [rearrange(W_i(x), 'b s (h d) -> b h s d', h = self.num_heads) for W_i in (self.w_q, self.w_v, self.w_k)]
...
class CrossAttention(nn.Module):
    ...
    def __init__(self, dim: int, head_dim: int = 64, qkv_bias: bool = False, proj_bias: bool = False):
        ...
        self.w_q_x = nn.Linear(dim,dim, bias=qkv_bias)
        ...
        # self.q_k_con = nn.Linear(dim, dim, bias=qkv_bias)
        self.q_kv_con = nn.Linear(dim,2*dim, bias=qkv_bias)
    ...
    def forward(self, x: torch.Tensor, context: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        # q = self.w_q_x(x) #x*W_t+b [B, N, dim]
        q = rearrange(self.w_q_x(x), 'b s (h d) -> b h s d', h = self.num_heads)
        # TODO: Compute the keys K and values V from the context. Each should be of shape [B num_heads M head_dim].
        kv = self.q_kv_con(context) #[B, M, 2*dim]
        k, v = kv.chunk(2, dim=-1) # 2X[B, M, dim]
